...
def get_config_and_setup(args):
    ...
    print(f"--- Ablation 2: Fully-Adaptive PGD Training ---")
...
# --- Full Defense Wrapper for Training and Validation Attack ---
class DifferentiableDefensePipeline(nn.Module):
    """
    A wrapper to make the full defense pipeline differentiable for an attacker.
    This is the target for both training data generation and validation, simulating
    a powerful, fully-adaptive white-box attack.
    """
    ...
    def forward(self, x):
        # The purifier must be in train mode for gradients to flow during attack
        if not self.training:
             self.purifier.eval() # Keep purifier in eval mode during validation attack
...
def main():
    parser = argparse.ArgumentParser(description="Ablation 2: Fully-Adaptive PGD Training")
    ...
    ablation_name = "pgd_fully_adaptive"
    ...
    # --- Attacker for BOTH TRAINING AND VALIDATION (Fully-Adaptive) ---
    # The attack is aware of the purifier and its current state.
    ...
    # Use a shorter attack for training to manage computation time
    train_attack = torchattacks.PGD(attackable_defense, eps=config.attack_params['eps'], alpha=config.attack_params['alpha'], steps=10)
    # Use a stronger attack for validation
    ...
    for epoch in range(config.joint_finetune_epochs):
        purifier_unet.train()
        attackable_defense.train() # Ensure the defense wrapper is in training mode
        ...
        for i, (x_clean, y) in enumerate(pbar):
            ...
            # --- Step 1: Generate Fully-Adaptive Adversarial Data ---
            # The attacker sees the full purifier->encoder->classifier pipeline.
            # This is the inner loop of adversarial training.
        ...
        # --- VALIDATION LOOP ---
        purifier_unet.eval()
        attackable_defense.eval() # Ensure the defense wrapper is in eval mode
